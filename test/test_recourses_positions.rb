...
class TestRecoursesPositions < IntegrationCase
  # A flat table — teams point nowhere — is positioned among the whole of itself, so its
  ...
  def test_an_positioned_table_stands_its_search_and_its_headings_down
  ...
  # A table pointing somewhere is positioned within what it points at, so the grip is
  ...
  def test_a_nested_index_is_positioned_and_the_same_model_listed_flat_is_not
    ...
    refute Recourse.positioned?(Step, nil)
    assert Recourse.positioned?(Step, Step.reflect_on_association(:team))
  end

  # The second listing of a positioned model, dragged into order by a column the model
  ...
  def test_a_host_may_position_a_listing_by_a_column_of_its_own
  ...
  # A model keeping no such column is a table nobody positions, whatever level it is
  # read at: no grip, and its headings and its search box stand as they were.
  def test_a_table_with_no_position_column_positions_nothing
    visit '/places'
    ...
    refute Recourse.positionable?(Place)
  end
end
