...
class TestRecoursesPositioning < IntegrationCase
  ...
  def test_a_drop_is_clamped_to_the_table_and_a_drop_in_place_writes_nothing
    ...
    assert_equal names.rotate(1), position_of(team)
  ...
  # A flat table is positioned among the whole of itself, so the route sits under the
  ...
  # The second listing of a positioned model writes the column its own page named and
  ...
  def test_a_hosts_own_listing_writes_the_column_it_positioned_by
  ...
  # A table nobody says is positionable is not one a drop can be reported to at all: the
  # route is drawn where the host asks for it and nowhere else, so there is nothing to
  # reach by hand and nothing to answer 404 with.
  def test_a_table_nobody_positions_draws_no_route_to_write_to
...
private
  ...
  def position_of(team) = team.steps.order(:position).pluck(:name)
