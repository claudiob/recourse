class CreateNotes < ActiveRecord::Migration[8.1]
  ...
  # A note is about one thing and points nowhere else.
  def change
    ...
    values = PER_ZIP.flat_map do |zip, count|
      (1..count).map do |number|
        "('ZIP', (select id from zips order by id limit 1 offset #{zip - 1}), " \
          "'Note #{number} about ZIP #{zip}.', current_timestamp, current_timestamp)"
      end
    end

    <<~SQL.squish
      insert into notes (about_type, about_id, body, created_at, updated_at)
      values #{values.join ', '}
    SQL
  end
end
